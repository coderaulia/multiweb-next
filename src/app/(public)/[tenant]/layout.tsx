...
import { resolveTenantByHost, resolveTenantBySlug } from '@/features/cms/tenantContext';
...
const fontBody = Sora({ subsets: ['latin'], variable: '--font-body', display: 'swap' });
const fontAccent = Playfair_Display({ subsets: ['latin'], variable: '--font-accent', style: ['italic'], display: 'swap' });
const fontTight = Inter_Tight({ subsets: ['latin'], variable: '--font-tight', weight: ['400', '500', '600', '700'], display: 'swap' });
const fontSerif = Instrument_Serif({ subsets: ['latin'], variable: '--font-serif', style: ['normal', 'italic'], weight: '400', display: 'swap' });
const fontMono = JetBrains_Mono({ subsets: ['latin'], variable: '--font-mono', weight: ['400', '500'], display: 'swap' });
...
async function resolveTenant(tenantSlug: string) {
  ...
  // Custom domain request: resolve by host
  if (tenantHost) {
    const tenant = await resolveTenantByHost(tenantHost);
    if (tenant && tenant.status === 'active') return tenant;
    return null;
  }

  // Path-based request: resolve by slug from URL
...
export default async function TenantLayout({ children, params }: TenantLayoutProps) {
  ...
  const pageNavMap = new Map(
    pages.map((page) => [
      page.id,
      {
        href: `/${tenant.slug}${page.seo.slug ? `/${page.seo.slug}` : ''}`,
        label: page.navLabel
      }
    ])
  );
  ...
  const siteSchema = {
    ...
    ...(settings.sitemap.includePosts
      ? {
          potentialAction: {
            '@type': 'SearchAction',
            target: `${settings.general.baseUrl}/${tenant.slug}/blog?query={search_term_string}`,
